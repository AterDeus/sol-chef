...
Пять пунктов таббара — те же лейблы, с иконками (§8). Не шестой пункт «Главная»: вход на `/` — липкая верхняя полоска бренда (иконка повара + «Кухонная шпаргалка»), в том числе на карточке рецепта; в cook mode полоска скрыта. «Калькулятор» и «На неделю» длинноваты в пять колонок — терпим. В справочнике сетка 2×2. Рецепт: ← Рецепты | Справка | Режим готовки (иконки — §8). Из слота набора «← Рецепты» можно вести на `/prep/<slug>`.

### Главная `/` — витрина, не калькулятор

Не каталог, не оглавление гидов и **не копия** `/calculator`. Подбор живёт на `/calculator`.

1. Eyebrow и заголовок **Кухонная шпаргалка**. Короткий лид. Две кнопки: **Подобрать блюдо** → `/calculator`, **Все рецепты** → `/recipes`. На ≤768 — отдельный кадр иллюстрации во всю ширину **под заголовком**, затем лид и кнопки. На ≥769 — две колонки (текст слева, натюрморт справа). Десктопный кадр с пустым полем слева на узком экране не кропать в квадрат справа от текста.
...
4. Низ футера: **Главная** · Крупы · Говядина · Свинина · Птица · Советы шеф-поваров · **Все рецепты**. Без «скоро».
...
Домашний раздел — по `protein_base` и роли на столе. Если чип меняет основу, та же карточка **может** появиться во втором белковом разделе (мясо / птица / рыба / овощи) с пометкой чипа и ссылкой `?variant=`, не вторым slug. Завтрак, десерт и гарнир во вторую главу не утекают. Коды VOCAB внутри раздела — узкий фильтр «Основа».
...
**Ещё варианты** — «Быстрее» только если минут меньше, чем у главного; иначе «На плите» / «В духовке» / «На несколько дней». Не Score. Ниже — ссылка **«Посмотреть все подходящие»** в книгу (глава из выбранного продукта; книга `have=` не принимает).
...
7. Два **отдельных** переключателя над ингредиентами, **не** внизу абзацем и **не** в одном списке чипов (9.1 + 9.2):
   - **Добавки:** если чипы меняют белок — **Курица · С говядиной** (чип базы = короткая домашняя основа). Иначе **Как в рецепте · С грибами · С пармезаном**. Один slug. Клик сразу меняет список, шаги, аллергены, заметки. Нет добавок — ряда нет.
...
- подпись **У меня**;
- − / поле / + ;
- единица всегда **г** (для объёмного якоря — **мл**); переключателя г↔кг / мл↔л нет;
...
**Взять жесты:** «У меня» (поле в граммах, factor, имя якоря, сброс, hint про таймеры; **без** г↔кг); cook mode setup + Заранее + ± таймера + обзор шагов + Wake Lock; таблица круп / карточки мяса; карточки советов (`hint` / `explanation`, не аккордеон секций); карточка без фото; токены `global.css`; Lucide 1px / спрайт; 301 со старого URL; empty-state; favicon.
...
| «У меня» единицы | **уточнено чатом 2026-09-21** — поле всегда в граммах (объём — мл); переключатель г↔кг / мл↔л снят |
